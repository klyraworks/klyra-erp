...
import { Rol } from "@/src/core/api/types"

export default function EditarRolPage() {
    const { id } = useParams()
    const rolId = id as string
    ...
    useEffect(() => {
        ...
        async function loadRol() {
            try {
                const data = await apiFetch<Rol>(`/api/seguridad/roles/${rolId}/`)
                setRol(data)
            } catch (err) {
                setError(true)
                if (err instanceof ApiError) alertas.error(err.mensaje, err.titulo)
                else alertas.error("No se pudo cargar el rol", "Error")
            } finally {
                setLoading(false)
            }
        }
    ...
    if (loading || isNavigating) {
        return <LoadingScreen message="Cargando Rol..." />
    }

    if (error || !rol) {
        return (
            <>
                <Header title="Error" breadcrumb={["configuracion", "Roles", "Error"]} />
                <main className="flex-1 overflow-y-auto p-6">
                    <div className="max-w-2xl mx-auto">
                        <div className="bg-card rounded-xl border border-border shadow-sm p-8 text-center">
                            ...
                            <p className="text-sm text-muted-foreground mb-6">El rol no existe o no tienes permisos para editarlo.</p>
                            <Link
    ...
    return (
        <>
            <Header
                title="Editar Rol"
                breadcrumb={["configuracion", "Roles", "Editar", rol.nombre]}
                actions={
                    <div className="flex items-center gap-2">
                        ...
                        >
                            ...
                            <span className="hidden sm:inline">Actualizar Rol</span>
                        </button>
                    </div>
                }
            />
            <main className="flex-1 overflow-y-auto p-6">
                <div className="max-w-7xl mx-auto">
                    <RolForm mode="edit" rol={rol} formRef={formRef} />
                </div>
            </main>
        </>
    )
