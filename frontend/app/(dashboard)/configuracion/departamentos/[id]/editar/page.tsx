...
import { Departamento } from "@/src/core/api/types"
import Link from "next/link"

export default function EditarDepartamentoPage() {
    const { id } = useParams()
    const departamentoId = id as string
    ...
    useEffect(() => {
        ...
        async function loadDepartamento() {
            try {
                const data = await apiFetch<Departamento>(`/api/rrhh/departamentos/${departamentoId}/`)
                setDepartamento(data)
            } catch (err) {
                setError(true)
                if (err instanceof ApiError) alertas.error(err.mensaje, err.titulo)
                else alertas.error("No se pudo cargar el departamento", "Error")
            } finally {
                setLoading(false)
            }
        }
    ...
    if (loading || isNavigating) {
        return <LoadingScreen message="Cargando Departamento..." />
    }

    if (error || !departamento) {
        return (
            <>
                <Header title="Error" breadcrumb={["Configuración", "Departamentos", "Error"]} />
                <main className="flex-1 overflow-y-auto p-6">
                    <div className="max-w-2xl mx-auto">
                        <div className="bg-card rounded-xl border border-border shadow-sm p-8 text-center">
                            ...
                            <p className="text-sm text-muted-foreground mb-6">El departamento no existe o no tienes permisos para editarlo.</p>
                            <Link
    ...
    return (
        <>
            <Header
                ...
                breadcrumb={["Configuración", "Departamentos", "Editar", departamento.nombre]}
                actions={
                    <div className="flex items-center gap-2">
                        ...
                        >
                            ...
                            <span className="hidden sm:inline">Actualizar Departamento</span>
                        </button>
                    </div>
                }
            />
            <main className="flex-1 overflow-y-auto p-6">
                <div className="max-w-7xl mx-auto">
                    <DepartamentoForm mode="edit" departamento={departamento} formRef={formRef} />
                </div>
            </main>
        </>
    )
