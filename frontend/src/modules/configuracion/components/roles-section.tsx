...
import { useRoles } from "@/src/core/store"
...
export function RolesSection({ compact = false }: RolesSectionProps) {
    ...
    const displayRoles = compact ? roles?.slice(0, 5) : paginatedData
    ...
    const handleEliminar = async (id: string, nombre: string) => {
        ...
        try {
            ...
            alertas.success("Rol eliminado exitosamente", "Rol Eliminado")
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-muted-foreground mt-0.5">
                            {isLoading ? "Cargando..." : `${totalItems} rol${totalItems !== 1 ? "es" : ""} registrado${totalItems !== 1 ? "s" : ""}`}
                        </p>
                    </div>
                </div>
            </div>
                ...
                {isLoading ? (
                    ...
                    <div className="py-16 text-center">
                        <div className="w-14 h-14 bg-muted/60 rounded-full flex items-center justify-center mx-auto mb-4">
                            <i className="fa-solid fa-shield-halved text-2xl text-muted-foreground/50"></i>
                        </div>
                        <p className="text-sm font-medium text-muted-foreground">Sin roles registrados</p>
                        <p className="text-xs text-muted-foreground/70 mt-1">Crea el primer rol para comenzar</p>
                    </div>
                ) : (
                    <>
                            ...
                            <table className="w-full text-sm">
                                <thead>
                                    <tr className="bg-muted/30 border-b border-border">
                                        ...
                                        <th className="text-left py-2.5 px-6 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Rol</th>
                                        <th className="text-left py-2.5 px-6 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Nivel</th>
                                        <th className="text-left py-2.5 px-6 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Grupos</th>
                                        ...
                                        <tr key={item.id} className="hover:bg-muted/20 transition-colors group">
                                            ...
                                            <td className="py-3.5 px-6">
                                                <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold bg-muted/60 text-foreground">
                                                    <i className="fa-solid fa-cubes text-[9px]"></i>
                                                    {item.total_grupos ?? 0}
                                                </span>
                                            </td>
                                            ...
                                            <td className="py-3.5 px-6 text-center">
                                                <DropdownMenu>
                                                    ...
                                                    <DropdownMenuContent align="end" className="w-48">
                                                        <DropdownMenuItem onClick={() => router.push(`/configuracion/roles/${item.id}/editar`)}>
